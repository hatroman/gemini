 ...
 * @returns {Array<Suite>}
 */
exports.flatSuites = function flatSuites(suites, parent) {
    if (!Array.isArray(suites)) {
        return [];
    }
    return suites.reduce(function(result, suite) {
        suite.path = [suite.name];
        if (parent && parent.path) {
            suite.path = parent.path.concat(suite.path);
        }
        result.push(suite);
        result = result.concat(flatSuites(suite.children, suite));
        return result;
    }, []);
};
